/*
  # Clean Database Functions and Create n8n Integration

  1. Purpose
    - Remove all existing HTTP-based triggers and functions
    - Create polling-based functions for n8n integration
    - Ensure proper question lifecycle management

  2. Functions Created
    - get_pending_questions: For n8n polling
    - mark_question_*: For status updates
    - reset_question_status: For admin management
    - get_question_stats: For monitoring

  3. Security
    - Proper RLS policies maintained
    - Function permissions granted appropriately
    - No HTTP calls from database triggers
*/
...
DROP FUNCTION IF EXISTS trigger_n8n_workflow() CASCADE;
DROP FUNCTION IF EXISTS log_new_question() CASCADE;
DROP FUNCTION IF EXISTS manual_trigger_n8n(UUID) CASCADE;
DROP FUNCTION IF EXISTS retry_failed_questions() CASCADE;
DROP FUNCTION IF EXISTS trigger_n8n_manual(UUID) CASCADE;
DROP FUNCTION IF EXISTS initialize_question() CASCADE;

-- Drop the problematic function with all possible signatures
DROP FUNCTION IF EXISTS get_pending_questions(INTEGER) CASCADE;
DROP FUNCTION IF EXISTS get_pending_questions() CASCADE;

-- Drop other potentially conflicting functions
DROP FUNCTION IF EXISTS mark_question_processing(UUID) CASCADE;
DROP FUNCTION IF EXISTS mark_question_completed(UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS mark_question_failed(UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS reset_question_status(UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS reset_question_status(UUID) CASCADE;
DROP FUNCTION IF EXISTS get_question_stats() CASCADE;
...
CREATE OR REPLACE FUNCTION mark_question_completed(question_uuid UUID, solution_html TEXT)
RETURNS JSONB AS $$
DECLARE
    updated_count INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION mark_question_failed(question_uuid UUID, error_message TEXT DEFAULT NULL)
RETURNS JSONB AS $$
DECLARE
    updated_count INTEGER;
BEGIN
